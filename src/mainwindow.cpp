...
void MainWindow::buildUi() {
    ...
    auto *infoLabel = new QLabel(
        ...
        "authentication dialog. Global shortcuts are supported on X11 sessions.",
        this);
    ...
    for (const auto &scenario : scenarios) {
        ...
        binding.global = new GlobalShortcut(this);
        connect(binding.global, &GlobalShortcut::activated, this, [this, scenario]() { setScenario(scenario.mode); });
        binding.global->setShortcut(scenario.defaultShortcut);

        binding.local = new QShortcut(scenario.defaultShortcut, this);
        connect(binding.local, &QShortcut::activated, this, [this, scenario]() { setScenario(scenario.mode); });

        shortcuts_.insert(scenario.mode, binding);
        if (!binding.global->isSupported()) {
            infoLabel->setText(
                "Root permission is required for every action. You may be prompted by your system's "
                "authentication dialog. Global shortcuts require X11 and are unavailable on Wayland.");
        }
    }
...
void MainWindow::updateShortcut(const QString &mode, const QKeySequence &sequence) {
    if (shortcuts_.contains(mode)) {
        auto &binding = shortcuts_[mode];
        if (binding.global) {
            binding.global->setShortcut(sequence);
        }
        if (binding.local) {
            binding.local->setKey(sequence);
        }
        if (!binding.global || !binding.global->isSupported()) {
            statusLabel_->setText("Updated shortcut (local only). Global shortcuts require X11.");
            return;
        }
    } else {
        ShortcutBinding binding;
        binding.global = new GlobalShortcut(this);
        connect(binding.global, &GlobalShortcut::activated, this, [this, mode]() { setScenario(mode); });
        binding.global->setShortcut(sequence);

        binding.local = new QShortcut(sequence, this);
        connect(binding.local, &QShortcut::activated, this, [this, mode]() { setScenario(mode); });
        shortcuts_.insert(mode, binding);
    }
